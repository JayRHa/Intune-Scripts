<#
.SYNOPSIS
    Automatically create assignment groups when a new Intune app is created.
.DESCRIPTION
    Monitors Intune audit events for new app creations and creates Available,
    Required, and Uninstall AAD security groups, then assigns them to the app.
.NOTES
    ...
    if ($_.activityType -eq 'Create MobileApp' -and $_.activityResult -eq 'Success') {
        ...
        foreach ($groupPost in $groupsPost) {
            try {
                ...
                Write-Warning "Failed to create group $groupPrefix-$appName-$groupPost : $_"
            }
        }
        foreach ($group in $groups) {
            ...
            try {
                ...
                Write-Warning "Failed to assign group $($group.displayName) to app $appName : $_"
            }
        }
    }
}
