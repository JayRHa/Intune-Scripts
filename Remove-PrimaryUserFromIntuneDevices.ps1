...
$sharedDevices = $allDevices | Where-Object {$_.deviceName -like $filter}

Foreach ($deviceEntry in $sharedDevices){
    Write-Host "Change $($deviceEntry.devicename) to a shared device"
    Delete-IntuneDevicePrimaryUser -IntuneDeviceId $deviceEntry.id -ErrorAction Continue
}
