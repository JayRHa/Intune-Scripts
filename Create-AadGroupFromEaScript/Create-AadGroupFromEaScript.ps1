<#
.SYNOPSIS
    Create AAD groups from Endpoint Analytics script output.
.DESCRIPTION
    Reads device health script results and creates or updates AAD security groups
    based on the detection script output (e.g. device manufacturer).
.NOTES
...
if ([string]::IsNullOrEmpty($tenantId) -or [string]::IsNullOrEmpty($clientId) -or [string]::IsNullOrEmpty($clientSecret)) {
    Write-Error "One or more credential variables (TenantId, AppId, AppSecret) are empty."
    return
}
...
try {
    ...
    Write-Error "Failed to retrieve device health script data: $_"
    throw
}
...
$result.value | ForEach-Object {
    ...
    try {
        ...
        $groupResult = Invoke-RestMethod -Uri $uri -Headers $authToken -Method GET
        if ($groupResult.value) {
            $currentGroupId = $groupResult.value[0].id
        }
        else {
            $groupResult = Add-MgtGroup -groupName $groupName -groupDescription "All $($_.preRemediationDetectionScriptOutput) devices (auto created via azure automation)"
            $currentGroupId = $groupResult.id
        }

        $uri = "https://graph.microsoft.com/beta/devices?" + '$filter=' + "startswith(displayName,'$deviceName')"
        $deviceResult = Invoke-RestMethod -Uri $uri -Headers $authToken -Method GET
        $deviceId = $($deviceResult.value[0].id)

        $uri = 'https://graph.microsoft.com/v1.0/groups/' + $currentGroupId + '/members'
        $memberResult = (Invoke-RestMethod -Uri $uri -Headers $authToken -Method GET).value | Where-Object {$_.id -eq $deviceId}

        if (-not $memberResult) {
            Add-MemberToGroup -groupId $currentGroupId -deviceId $deviceId
        }
    }
    catch {
        Write-Warning "Failed to process device $deviceName : $_"
    }
}
